...
Maybe a locally optimal solution is good enough, if it's better than the solution
human experts can come up with. Maybe if you throw enough hardware, sample
inefficiency stops mattering. The human preferences paper suggests that it's
possible to define well-shaped reward functions implicitly by rating behaviors
as good or bad, instead of trying to define a good reward yourself, which is a good
sign because it's 
