...
ago, and was the algorithm behind the famous [parkour paper](https://www.youtube.com/watch?v=hx_bgoTF7bs).

VIDEO

These results are really impressive, but based on the paper it took about 100
hours of training with 64 workers to learn the policy for the Humanoid.
In comparison, back in 2012 online trajectory optimization could solve these
...
To be honest, I really didn't understand why this even merited a blog post. Of
course 
like that this was billed as "faulty reward functions in the wild", because it
felt so freaking obvious that this happens already.
