...
Deep reinforcement learning is driven by mountains and mountains of hype. It
deserves a lot of it. Reinforcement learning is incredibly flexible paradigm,
and in principle, a robust and performant RL system should be great at
everything. Merging this with the power of deep learning seems like the closest
thing we have to any sort of AGI. That's the kind of dream that fuels billions
...
test out: let me tell you many of the reasons why reinforcement learning
is terrible.
...
Below is a video frmo the famous [parkour paper](https://arxiv.org/abs/1707.02286),
trained using PPO.
...
Below is a video of robots similar to ones in the MuJoCo benchmarks, controlled
with online trajcetory optimization. This video is from 2012, and the behaviors
are more impressive than the majority of reinforcement learning results. The
correct actions are computed in near real-time, online, with no offline
training.
...
How can this do so well? The approach used (model predictive control) gets to
do planning against a known, ground-truth world model. So on one hand, it's
not a fair comparison to model-free RL. On the other hand, it's somewhat
...
if you want to solve anything with reinforcement learning, you're better off
by avoiding it unless absolutely necessary. Hence my 70% claim from before.

Take Boston Dynamics. They rather legendarily gave a presentation at NIPS, a
leading machine learning conference, and then said on stage that they don't
do machine learning. And upon further digging, I found a paper from 2015 that
mostly talks about optimization and time-varying LQR. I would argue those
techniques are close enough to machine learning, but they're not what most
people expect.
...
Anecdotally, I've heard several more horror stories, mostly tied to small,
perverse properties of black box optimization.
...
A friend mentions training an agent to play a racing flash game. The reward
is defined by applying OCR to the pixels on the screen to parse the score.
Unfortunately, the OCR isn't perfect. In initial exploration, the OCR made a
rare, but important mistake - it interpreted the start line of the race as
a leading 1, which increased the score by an order of magnitude. This
completely killed learning, because the policy spent the rest of its time
trying and failing to reproduce the same error.
...
I know people like to tell stories about paperclip optimizers, and it's an
important concept, but honestly, I'm sick of hearing about it.
There's no reason to speculate that far when present-day examples of misaligned
incentives show up all the time, including in current reinforcement learning
research.
...
RL algorithms.) The environment is HalfCheetah. The reward is modified to
give 0 reward until the agent runs forward ??? units, to make exploration
harder. The y-axis is episode reward, and the x-axis is number of timesteps,
in thousands.

![Plot from VIME paper](/public/rl-hard/vime.png)

The dark line is the median performance over 10 random seeds, and the shaded
region is the 25th to 75th percentile. On one hand, it is nice that VIME helps
on this problem. But on the other hand, when you consider how close the 25th
percentile is to zero, it's disheartening to realize that it's failing
10-20% of the time for no reason.
...
Even When You Get Good Results, It's Often Just Overfitting to Weird Patterns In the Environment
==============================================================================

In RL, your train set is your test set. The upside is that if you want to do
...
Consider the Atari benchmark. DQN can solve a lot of the Atari games, but
