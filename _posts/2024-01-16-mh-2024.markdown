...
## The tph-site Rabbit Hole

I am sure TTBNL will get asked about tech during the AMA, but I have an unusually informed view on what happened - or
rather, the extent it's known what happened.
...
When the hunt site locks up and fails to recover, everyone who worked on tech infra knows *exactly* what's happened -
traffic is eating up all the database connections and not letting go. This has been a persistent problem with tph-site's usage
of websockets via [Django Channels](https://channels.readthedocs.io/en/latest/), where it's super hungry on database
connections. We've never fully resolved this, but intuitively there's no way a site with a few thousand concurrent users
should need over 500 Postgres connections. It just...no, something smells wrong there on the math. It's caused issues for us
in the past, but we've worked around the issue by using [PgBouncer](https://www.pgbouncer.org/) to do [connection pooling](https://stackoverflow.blog/2020/10/14/improve-database-performance-with-connection-pooling/), then using load tests to verify we can open enough database connections to handle the load.

We're pretty invested in getting Hunt fixed, so a few teammate people drop into the handoff server to help debug. There isn't
too much we can do aside from passing along sample queries we used for diagnostics, asking questions about server state
and configs, and recommending ways to reduce Websocket load.

Over a few back and forths, we find the following:
...
This makes debugging the issue really hard, since there are no logs, there is no reproduction setup aside from prod, and
the server isn't hitting any obvious bottlenecks. And so the best routes we can recommend to TTBNL are to just strip out
all non-essential Websockets and apply random changes to the database config until something works.

While reading PgBouncer documentation beteen deploys, those of us on the teammate side start wondering if our PgBouncer config numbers are
wrong (we seem to have `default_pool_size == max_client_conn` but this doesn't make much sense). We consider telling TTBNL
to try removing the PgBouncer layer entirely, but decide not to because we're not confident it'll help or do anything.
Meanwhile, TTBNL independently decides something is fishy enough with PgBouncer to try a deploy without it.

Surprisingly, this *works* and the site stops failing immediately.
</div>
...
We very definitely did not want to win this year (it wasn't even asked on the team survey),
and so we had a #losecomm to figure out a way the most reasonable way to do so while still having fun.
...
**Herc-U-Lease** - Ah, the scavenger hunt! We started doing this, but did a cost-benefit analysis and decided the effort
