...
the answer had to be a two digit number, so we took everything mod 100. *Then*,
instead of extracting the digits N places after the Feynman point, we thought we
had to find where those digits appeared in $$\pi$$ after the Feynman point.
Somehow, the first 8 numbers we extracted this way were all even, and we spun in
circles for a while until someone tried the obvious thing instead.
