...
before trying it for the next puzzle, and make sure each puzzle has at most 1
outstanding submission, but otherwise backsolve as much as you want.
Historically, this means our solve accuracy is lower than other teams.
...
be a real tea blend! Oops. Eventually they cut open the tea bags and tried to
identify ingredients by phonelight.
...
![We See Thee Rise](/public/mh19/seetheerise.png)
{: .centered }
...
day job, I was embarassed to learn that I didn't have an on-hand way to
...
normalize the image array, which explains why we were getting high-confidence
misclassifications of the wrong class.
...
Somehow, all the
partial work we had gave an even number, so it looked like *something* was
...
puzzle, since it could have been about the color pie.
Then someone remembered that Trivial Pursuit is
another game with colored wedges and the colors matched the givens for the
Pi-Holi meta. Oops.
