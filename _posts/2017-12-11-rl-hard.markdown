...
of search. But, it does show that if you just want a good Atari bot, spending
a bunch of time training DQN may not be your best bet.
...
could help with learning. That leads to correspondingly low sample efficiency.

The rule-of-thumb is that except in rare cases, domain-specific algorithms will
...
subtle, and the end result isn't always that impressive.
...
accept that there was some value from that blog post.
...
and yes, this works, but there's a key problem.
...
The final model used is the model trained with both supervised and reinforcement learning,
even though it's ROUGE score is smaller.
...
Here's another fun example. In
[this paper from DeepMind](https://arxiv.org/abs/1704.03073),
the authors use a distributed version of DDPG to learn a grasping policy. The
goal is to grasp the red block, and stack it on top of the blue block. For the
initial lifting motion, reward is given based on how high the red block is.

However, importantly, the reward is defined by the z-coordinate of the
...
Anecdotally, I've heard several horror stories of trying to fight RL with
reward functions. It's been known for a while
...
to deviate from this policy in a meaningful way - you have to take several
exploration steps to stop the rampant spinning first. It's certainly
learnable, but in this run, it didn't happen.

In other words, it's another case of the classic exploration-exploitation problem that has dogged
...
(Ref: http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.57.1916&rep=rep1&type=pdf)
