...
unambiguous win for deep RL. It's a surprisingly rare occurence.

And it's also harder for me to explain to laypeople why my problems
...
those classical techniques still work pretty well.
...
so you have never have to worry about defining your reward, and you know everyone
else has the same reward function.

This is also why the MuJoCo tasks are popular. Because they're run in simulation,
you have perfect knowledge of all object stat, which makes reward function design
a lot easier.
...
Reward hacking is the exceptions. The much more common case is a poor local optima
...
**Use reinforcement learning just as the finetuning step:** The first AlphaGo
paper started with supervised learning, and then did RL finetuning on top of it.
This is a nice recipe, since it lets you use a faster-but-less-general method
to speed up initial learning. It's worked in other contexts - see [Sequence Tutor (Jaques et al, ICML 2017)](https://arxiv.org/abs/1611.02796). You can view this as starting the RL process with
a reasonable prior, instead of a random one.

(I will add the Yann LeCun cake picture here. I really tried to avoid mentioning
...
a data-driven way to generate reasonable priors. Earlier, I said that one problem
with RL is that it tries to do everything.
We don't need RL to solve everything. We just need it to solve real-world
tasks. There should exist a
a real-world prior that lets us quickly learn new real-world tasks, at the cost
of general learning ability, but that's a perfectly acceptable trade-off.
