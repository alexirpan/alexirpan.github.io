...
(A final aside: many of the following are problems in not just deep RL, but
RL in general. However, I place special emphasis on deep RL because my
first-hand experience is biased towards neural net approaches.
The hype of RL is driven by the
promise of applying RL to large, complex environments where good function
approximation is necessary. It is that hype that needs to be addressed.)
...
of the several incremental advances made to DQN in the past 5 years.
...
paper (Henderson et al, AAAI 2018). It makes a bunch of points that I think were well-known in the RL
...
(Add a note about "optimization through differentiation"?)
...
**Reward functions could be learnable:** There's been a lot of work in
imitation learning and inverse RL to have reward functions be learned implicitly
from demonstrations of good behavior. Working along this axis
could let use sidestep many of the difficulties that come from explicit reward
defintion. There are several recent papers to this effect: Learning from Human Preferences, Guided Cost
Learning, Time-Contrastive Neural Networks, (that Berkeley paper from the RL
symposium.)
The human preferences paper in particular found that for some
tasks, a reward implicitly defined by human reward outperformed the
existing reward definition, because humans ratings were better for
encouraging partial progress that wasn't covered in the hardcoded reward.
(ELABORATE BETTER, MENTIOIN TAMER?)
...
someone asks me whether RL can solve their problem? I'll tell them no, it
can't. But I'll also tell them to ask me again a few years from now. By then,
maybe it can.
