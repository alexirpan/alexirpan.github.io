...
tears that go into making deep reinforcement learning work.

Several times, I've seen people try deep reinforcement learning for the first
time. Without fail, they underestimate deep RL's difficulties.
...
I don't think deep reinforcement learning is useless, and I don't think it
doesn't deserve further research, and I certainly don't want to discourage people
from joining the field. I just want people to know what they're getting into,
so that if they fail to get positive results for a month, they know it's not
necessarily their fault.

The following is a collection of faults in current deep RL techniques, aggregated
across several papers from the past 5 years. To make things very clear: **these
are all good papers.** They make good research contributions, and many of them
are directly addressing the faults of deep RL. In several cases, I only cite
the paper because their baselines form a compelling negative result.

A final note: in the rest of this post, I use RL and deep RL interchangeably.
However, I'm criticizing just deep RL in particular, since the cited works
represent the agent with a deep neural net. These criticisms *may* apply
to linear RL or tabular RL, but I'm confident enough to generalize that far.
...
human or superhuman performance in several (but not all) Atari games.

Atari games run at 60 frames per second. Now let's do an experiment. Off the
...
you think it will. I'm starting to believe reinforcement
learning has its own planning fallacy - it usually needs more samples than you
think it needs.

This is not an Atari-specific issue. The 2nd most popular benchmark is the
MuJoCo benchmarks, a collected of simulated robots in the MuJoCo physics
simulator. These benchmarks take about $$10^5$$ to $$10^7$$ steps to learn,
and the networks used are often fairly small 2-layer or 3-layer fully
connected networks. This is an astoundingly large amount of experience to
control environments where you don't need computer vision and are given
ground-truth information about all joint positions and velocities.
...
that deep RL was able to learn these running behaviors like this. At the same
time, the fact that we needed to use 6400 CPU hours to learn these behaviors
is a bit disheartening. It doesn't feel like these gaits should require that
much computation to learn.
...
On the front of purely getting good performance, deep RL's track record isn't
...
These behaviors are also really cool! I think they compare well to the parkour
paper. So what's different between this paper and that one?

The trick is that Tassa et al use model predictive control, which gets to
perform planning aganist a ground-truth world model (the physics simulator).
Model-free RL doesn't get to do this planning, and therefore has a much harder
job. On the other hand, if you have a model, and planning against a model
can entirely eliminate the need for offline training, why bother with all
the bells and whistles of training an RL policy?

In a similar vein, suppose you just wanted to make a really good Atari agent,
and you didn't care how you did it. If that's true, you shouldn't use DQN,
you should use Monte Carlo Tree Search. Here are baseline
...
a paper that investigated ways to combine DQN with MCTS. They report the score
acheived by a trained DQN agent, and an agent that uses UCT, 
the standard version of MCTS.
...
you're doing reinforcement learning for reinforcement learning's sake, but personally,
I find it's harder to stay motivated when I compare RL's performance to, well,
anything else. And it's also harder for me to explain to laypeople why my problems
are cool and hard and interesting, because they often don't have the context
or experience to appreciate *why* things are hard.

I'm working in robotics right now. Consider the company most people think of
...
In other words, they mostly apply classical robotics techniques.

The goal of the robot learning community is to show machine learning can
surpass classical robotics techniques, making it possible to learn behaviors
that apply to general environments.

No one at Boston Dynamics has to explain why teaching a robot to do that is cool.
It is objectively cool. I mean, *my goodness*.
Nothing I work on is near that visually impressive. "Oh, you're teaching robots
to pick things up? Uh, cool, I guess."
I have to explain why my problems all the time, because often they're hard for
subtle reasons that don't sound like a big deal if you're missing research
context.

![xkcd comic](/public/rl-hard/xkcd_tasks.png)
{: .centered }

(From xkcd. For the record, this [isn't a 5 year problem](http://code.flickr.net/2014/10/20/introducing-flickr-park-or-bird/),
but the message is on point.)
{: .centered }


Reinforcement Learning Usually Requires a Reward Oracle
==============================================================================

Reinforcement learning assumes the existence of a reward function. Usually,
this is either given, or it is handtuned offline and kept fixed over the course
of learning.
...
do things you don't expect. This is why Atari is such a nice benchmark. Not only
is it easy to get lots of samples, the goal in every game is to maximize score,
so you have never have to worry about defining your reward.

Most RL results come from simulated environments,
in part because perfect knowledge of the state makes reward function design
waaaaaay easier. As an example, here are some reward functions for the
MuJoCo benchmarks.
...
give reward at the goal state, and no reward anywhere else. Shaped rewards
are often much easier to learn, because they provide positive feedback,
even if the policy hasn't figured out a full solution to the problem.

Unfortunately, in practice, this reward shaping can bias learning, leading
...
The provided reward gives points for hitting target locations, and also
...
doesn't always match what you want.**
...
of misspecifed reward *was* the boat racing video. And since then, that video's
...
necessary, but I've never felt like I've learning anything by doing it.
...
I've heard several anecdotes to this effect from other RL researchers
...
These are the exceptions. The much more common case is a poor local optima
that comes from getting the exploration-exploitation tradeoff wrong.
...
The ideas have been around since the 1980s and earlier, and several of them have
been attempted with deep learning models. There's been some preliminary success,
but as far as I know, none of them work consistently enough to justify adding
them by default. I'm skeptical a silver bullet of that caliber will be figured
out anytime soon. Not because people aren't trying, but because exploration-exploitation
...
but it doesn't give the wild success people empirically get from
...
I find this work very promising, and I give more examples in a later section
discussing future work. However, in practice, I think the
generalization capabilities of RL aren't strong enough yet. Deep RL has yet to
...
We studied a toy 2-player game, where there's a closed-form analytic solution
for optimal play, and where there's a tunable difficulty parameter.
...
of the environment. By trained player 2 against the optimal player 1, we showed
RL could learn good strategies. But we also found that if we deployed the same
policy against a non-optimal player 1, it failed to generalize and did poorly.
...
Deep RL isn't stable at all, and it's just hugely annoying for research.
...
If my supervised learning code failed to beat random chance 30% of the time, I'd have
...
effectively. It's all a bit depressing.
...
(TODO: add RL + SL methods? Papers mentioned: Finetuning RNNs for music
generation, NTMs, etc.)
...
RC car at NIPS and said it used reinforcement learning. I know there's some
neat work
[optimizing device placement for large Tensorflow graphs (Mirhoseini et al, ICML 2017)](https://arxiv.org/abs/1706.04972).
Salesforce has their text summarization model, which worked if you massage the
RL more carefully.
...
be strong.) Every Internet company ever has probably thought about adding RL to
their ad-serving model, but if anyone's done it, they've kept quiet about it.

Overall, I'm pessimistic that people have gotten RL to work and are simply
not publicizing it. It's more likely that deep RL is still a research topic,
and still not robust enough for most applications.
...
* [A Super Smash Brothers Melee bot](https://arxiv.org/abs/1702.06230) from students at MIT and NYU, that can beat
...
* **There's a clean way to define an ungameable reward.** Two player games
...
We can combine a few of these principles to analyze the success of Neural
Architecture Search. As mentioned above, the reward is validation accuracy.
...
**Good priors could heavily reducing learning time:** This is closely tied to
the transfer learning point. In one view, transfer learning is about using
past experience to build a good prior for learning other tasks.
RL algorithms are designed to apply to any Markov Decision Process.
However, we often want to solve a small section of environments.
...
This very much seems like the future - if such a metalearned prior is possible.

A summary of recent learning-to-learn work can be found in
[this post](http://bair.berkeley.edu/blog/2017/07/18/learning-to-learn/)
