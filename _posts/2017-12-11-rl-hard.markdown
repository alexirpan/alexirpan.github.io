...
towering garbage pile of pain, failed experiments, statistically insignificant
results, and pure, nonsensical bullshit.
...
until they set their research expectations realistically.
...
they do nothing for a month, they know it isn't their fault.
...
RL or tabular RL, but I'm not confident enough to claim they generalize.
The hype of RL is driven by the promise of applying RL to large, complex
environments where good function approximation is necessary. It is that hype
...
Now, I like the parkour paper a lot. But, it's worth putting it in context of
prior work.
...
training. Oh, and it's from 2012. ([Tassa et al, IROS 2012](https://homes.cs.washington.edu/~todorov/papers/TassaIROS12.pdf)). So it's also running on 5 year old hardware.
...
not a fair comparison to model-free RL. On the other hand, given that a model
helps by this much, why would you spend a bunch of time training an RL policy
that gets results that look *worse* than this?

In a similar vein, you can easily outperform
DQN in Atari with off-the-shelf Monte Carlo Tree Search. Here are the numbers
from [Guo et al, NIPS 2014](https://papers.nips.cc/paper/5421-deep-learning-for-real-time-atari-game-play-using-offline-monte-carlo-tree-search-planning). UCT is the standard version of MCTS
used today.
...
Again, not a fair comparison, because DQN does no search, and MCTS does a lot
of search. However, it does show that if you just want a good Atari bot,
throwing a tree search algorithm at the problem is probably easier than training
a DQN.
...
work faster and better than reinforcement learning. This isn't a problem if you're
doing RL for reinforcement learning's sake, but it can make it harder to get
motivated, and it's harder to explain to laypeople why the problems you're
working on are cool.
...
presentation where they said they din't do machine learning.
If you look up research papers from the group, you can find
[that they use time-varying LQR, QP solvers, and convex optimization](https://dspace.mit.edu/openaccess-disseminate/1721.1/110533). In other words, classical robotics stuff.
...
No one at Boston Dynamics has to sell why teaching a robot to do that is cool.
I have to sell my problems all the time, because the reason they're hard is
subtle, and the end result isn't always as visually impressive.
...
it than necesssary. How is it news that reinforcement learning does weird
...
deal with non-differentiable rewards. They tried using RL to optimize ROUGE,
and yes, this works, but it turned out high ROUGE doesn't always give you
good summaries.
...
isn't expected to generalize to other games.
The model isn't even expected to be a good initialization for a new Atari
game. In contrast, if you're doing image classification,
...
Here's a plot from the [Variational Information Maximizing Exploration paper (Houthooft et al, NIPS 2016)](https://arxiv.org/abs/1605.09674).
...
a recent paper,  [Deep Reinforcement Learning That Matters (Henderson et al, AAAI 2018)](https://arxiv.org/abs/1709.06560). It makes a bunch of points that I think were well-known in the RL
...
reinforcement learning to figure out a path to high reward.
    ...
    As a case example of this, let's examine the [original neural architecture search paper (Zoph et al, ICLR 2017)](https://openreview.net/forum?id=r1Ue8Hcxg). The reward is validation accuracy, which is a
...
(Mention transfer learning here! UVFAs, etc.)
...
seen, model-based approaches use fewer samples as well.
    ...
    Or to be more precise, they use fewer samples when they work. Learning good models
is hard. My impression is that low-dimensional state models work sometimes, and image
models are usually too hard. But, if this gets easier, some interesting things
could happen.

Papers of interest from the Berkeley robotics labs: [Guided Policy Search (Levine et al, ICML 2013)](https://graphics.stanford.edu/projects/gpspaper/gps_full.pdf), [Neural Network Dynamics for Model-Based Deep RL with Model-Free Fine-Tuning (Nagabandi et al, 2017](https://people.eecs.berkeley.edu/~nagaban2/resources/paper_mbmf.pdf),
[Deep Spatial Autoencoders for Visuomotor Learning (Finn et al, ICRA 2016)](http://rll.berkeley.edu/dsae/dsae.pdf),
[Self-Supervised Visual Planning with Temporal Skip Connections (Ebert et al, CoRL 2017)](https://arxiv.org/abs/1710.05268), [Combining Model-Based and Model-Free Updates for Trajectory-Centric Reinforcement Learning (Chebotar et al, ICML 2017)](https://arxiv.org/abs/1703.03078).
...
that the learned reward was actually better-shaped for learning than the baseline
reward used for that task, which is an interesting practical finding.

For recent work that doesn't use deep learning, see [Inverse Reward Design (Hadfield-Menell et al, NIPS 2017)](https://arxiv.org/abs/1711.02827) and [Learning Robot Objectives from Physical Human Interaction (Bajcsy et al, CoRL 2017)](http://proceedings.mlr.press/v78/bajcsy17a/bajcsy17a.pdf).
...
perspective, the issues of RL may not matter for practical purposes. Suppose
a finance company is using RL. They train a trading agent based on trades in
the US stock market, using 3 random seeds.
