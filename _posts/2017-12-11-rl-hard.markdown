...
subtle reasons that aren't as visually impressive, unless you have enough
research context.
...
from the end of the arm to the target, plus a small control cost. If you want
to do the same in the real world, you need very accurate positions, and good
sensors for that cost money.
...
These are both *shaped* rewards, meaning they give increasing reward in states
...
Unfortunately, shaped rewards tend to bias the learned policy, and can reward
behaviors that don't match what you want.
A good example is the boat racing game, from an [OpenAI blog post](https://blog.openai.com/faulty-reward-functions/).
The intended goal is to finish the race, but you also get points for collecting
powerups, and finishing the race gives less points than collecting powerups.
...
was making an unnecessarily large deal out of that point.
...
My grumpiness aside, there are other examples of improper reward functions.
One such example comes from text summarization. as seen in this
[blog post from Salesforce](https://www.salesforce.com/products/einstein/ai-research/tl-dr-reinforced-model-abstractive-summarization/).
...
ROUGE directly. This does end up giving high ROUGE, but high ROUGE doesn't
guarantee the summaries are high-quality.
...
haven't seen before, their performance drops. Note
...
Seven of these runs worked. Three of these runs didn't. *Turns out a 30%
...
if my hyperparms are bad, or if reinforcement learning is just messing with me. Maybe it's just Tuesday.
...
(TODO: add RL + SL methods? Finetuning RNNs, RL used in NTMs, etc?)
...
(TODO: unify model-based, extra learning signal, and priors together?)
