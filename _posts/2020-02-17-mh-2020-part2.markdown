...
into one post, to motivate me to finish it once and for all. This post is the result.
...
in a Mystery Hunt. I don't think Mystery Hunt should confirm partials, and
I do think a stronger rate limit make sense. The reason we confirmed partials
with a generous rate limit was to make our hunt easier, but puzzles shouldn't
need partial confirmations to have a clear final answer, and MIT Mystery Hunt
doesn't need the guardrails that partials give you.
...
Food Court had one of these all-or-nothing extraction mechanisms, because
...
transcribed, we were able to jump right into it. I had DUBSTEP for modernized
...
groans cascade across the space as each room discovered the puzzle 5 seconds
after each other.
Out of Puzzle Traumatic Stress Disorder,
no one worked on it, and we didn't even backsolve it until after getting to the
Workshop metameta.
...
way for teams to add help in rounds they need help on. Our strategy was that
...
had two options. One: use it right away on Cascade Bay. Or two: wait to see if
...
an hour, so we spent the pass on Cascade Bay. It said we used it, but nothing
happened.
...
out all three meta mechanics, and didn't need the help.

teammate was tied with Galactic until Cactus Canyon. We got stuck early, and
they didn't.
...
Workshop metameta first, but there's a lot of what-ifs here.
...
they wanted the answer to end in NEUTRALITY, and I moved
...
given clues weren't even the extraction order. In effect, my contribution was
convincing other solvers we were making progress.
...
A few people had monopolized one of the rooms for several
...
[Wanted: Gangs of Six](https://pennypark.fun/puzzle/wanted_gangs/) (IDed the ponies, Big Hero 6, some
Watchmen, then left).
...
those know, the minipuzzles got a lot easier.
...
but missed LIGHTNING DUST (she's a key part of two episodes, but she's
also an asshole).

We got GRACE UNDER FIRE as part of extraction, and that gave me an epiphany.
...
The people picking up pennies for Safari and Creative Studios came back, and
announced that a team had just solved the Cactus Canyon meta. "Wait, what?"
Here's how it happened: as they left the Workshop, they heard the solve jingle
for Cactus Canyon from the computer of the person running the Workshop. Why
would that happen? Well, suppose a team solved a meta. Left Out would want to
prepare for that team picking up their penny. What if they reused the solving
jingles to let the Workshop person know? So therefore, someone had just solved
Cactus Canyon, and were about to start the endgame.
...
and confirmed that our penny pick-up matched their Cactus Canyon solve time.
...
go through them. This led to some tense moments - on opening [Cowboy Campfire](https://pennypark.fun/puzzle/cowboy_campfire/), many of us wanted to immediately try backsolving with
HOMECOOKED MEAL for theme reasons, but were overruled because the number of
clues didn't match up.
...
two people duplicated a sheet at the same time, leading to conflicting
sheet names. Eventually, we got something that looked very promising, told everyone
to move to "Copy of Copy of Copy of Nathan's sheet", and solved it. The answer
...
data, the phrases were tricky to find if you didn't know what you were looking
for, and you needed 2 or 3 phrases for a single penny to be confident in
what to do. It just didn't play to our strengths very well. We have really good
...
14! permutations. Combining clever permutation order with the 1 in 7 filtering
definitely made it fit within brute-force range.

I'm pretty sure this would have worked if we coded it up. We didn't, but I
mention it because 3 people, all new to the puzzle, independently asked if the
brute force was possible. That's just how we are as a team.
[Number Hunting](https://www.puzzlesaremagic.com/puzzle/number-hunting/) from
Puzzles are Magic testsolved pretty fast among testsolvers from teammate,
because most
of them had the same realization - once you solve enough to find the list of
candidate answers,
it's much faster to export the data and use regexes to search for
matching answer enumerations, instead of solving the clues. When I checked the stats for
Number Hunting, I was not surprised that a teammate team got the fastest solve.
...
(This, by the way, is why the wrap-up presenters kept saying "SPOILER FREE ANSWER PHRASE".
The only person spoiled on Workshop's answer was the person Left Out called.
When we told this to Left Out, they told us they'd avoid saying the answer to
Workshop during the finale and wrap-up, so that we could finish it ourselves.)
...
Park. I know there are videos of the finale at wrap-up. What that video doesn't
...
broadcasted to you from twelve screens covering every wall.
...
to write, but not everyone. The team is reckoning with an uncomfortable chain
...
from Galactic. It will be less \*WHOOSH\* Galactic Trendsetters \*NYEEEEOW\* and
more \*WHOOSH\* *please let us sleep \*ZZZZZZZ\*.
...
solvers know what good puzzles look like, but that still leaves knowing how
to create good puzzles, on top of all the other organization involved.
...
it happen. The most direct consequence of our strong performance this year has
been serious discussions about how teammate can build more puzzle writing
expertise.

And, that's it. Jeez, this got really, really long. For puzzlers looking for
new hunts,
