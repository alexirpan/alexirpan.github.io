...
It took me a while to start writing this post, because my feelings on AI safety are complicated,
...
to make his comparative advantages better. I agreed, and it does push against leaving
robotics. However,
as I've argued [in my previous post]({% post_url 2024-07-08-tragedies-of-reality %}), I expect my experience with
...
And, well, I talk a big game about impact, but I shouldn't need to do so to justify
myself. I'm not
...
(I tried to get an LLM to make this for me and it failed. Surely this is possible with the right prompt?)
{: .centered }
...
There's often conflation between AI safety the research
...
4. I'm not sold our current tooling scales to better systems, especially superhuman systems.
...
how likely is it to be successful, and so on. I ended up with around 2%. I am the kind of person who thinks 2% risks are worth looking at.
...
is a chance that alignment is overblown, existing common sense in LLMs will scale up, models will generalize intent correctly,
and OSHA / FDA style regulations on deployment will capture the high-risk scenarios.
I also think there is a chance that doesn't happen. There are scenarios where you
want to allow some rule bending for the sake of innovation, but to me AI is a special enough technology
that I'm hesitant to support a full YOLO "write the regulations after we've spilled enough blood" strategy.

I also don't think we have to get all the way to AGI for AI to be transformative. This is
because I buy an argument made by [Holden Karnofsky](https://www.cold-takes.com/ai-could-defeat-all-of-us-combined/), that if a lab has the resources to train an AI, it has the resources to run millions of copies of that AI at inference time, enough to outnumber naive human supervision
if run over a wide enough set of contexts.
...
People are too [hungry]({% post_url 2023-07-19-ml-hurry %}) for the best models to do due
...
Fairness in Machine Learning side too, but a lot of existing literature focuses on fairness in
...
*so much* work to do.
Even if the sociology is quite different, the fundamentals of both areas are about
...
of models without corresponding safety guarantees.
...
people you're not. I don't know, I think most people I talk about safety with
are either genuine, or confused rather than insincere.
Aiming for safety while confused is better than not aiming at all.

So, that's what I'm doing. Aiming for safety. It may not be permanent move, but it feels right based on the current
climate. If that climate turns to AI winter, I will reconsider. Right now, it is *very* sunny
