...
is a vibes-based adjustment. Please hold all criticism that this is not a well-founded reasoning
strategy, I already know it isn't. If you want to disqualify all AI predictions based on fuzzy
principles, you'd have to throw out 90% of writing on the subject.

The TL;DR reasoning is that in 2020, I noted some trends that
made me think near-term AGI was unlikely but more plausible. Now, some events I considered
unlikely came to pass, and that is pushing probability mass closer in time.
I think it would be most instructive if I directly compared my 2020 post to my new one, because
it's a fun time capsule into what things were like 4 years ago.
...
Do I get to say I called it? The view of "things emerge
at scale" is significantly more mainstream these days. I totally called it.

Well, to be fair, I think I only get to claim a partial win. I assumed
next token prediction would be hard to use in new use cases.
...
if you finetuned on enough "instruction: example" data, and LLMs are good enough
at pretending to make it close enough to actually following intent. The same supervised
loss was good enough, and it was not much more difficult than that.
...
In 2020, I did an exercise I called "trying hard to say no". I decided to start from
...
out of 100, with 100 matching peak search interest.

![Artificial intelligence chart](/public/ai-timelines-2023/artificialintelligencesearchtrend.png)
{: .centered }
