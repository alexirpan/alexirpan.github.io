...
> Generative AI is creating spam, polluting its own data well, and will eventually choke its own growth.

This
[article from Scientific American](https://www.scientificamerican.com/article/ai-generated-data-can-poison-future-ai-models/) asks whether generative AI is making itself harder to train. For
...
Right now, I think this is important to be aware of, but will get worked around in the short
term. The entire "AI self-play" thesis assumes that we have reward models that can
separate "good" data from "bad" data. So far I don't have reasons to believe that's impossible.


## Overhangs Bear

> We've caught up on all the big overhangs and the rate of progress will slow down.
...
increase in capabilities as researcher quickly assemble the right ideas together and fill
in the overhang.

I think this view of the world is correct in describing how technology evolves, but doesn't
...
what day-to-day work is in research. Progress tends to look like a series of sigmoid
curves, where every so often you go through an inflection point. Making a projection based on
what the curve looks like at the inflection point is wrong, but making a projection based on
the flatter region after the inflection point is wrong too.

![Sigmoid curve](/public/mh-2023/sigmoid.png)
{: .centered }

As I mentioned earlier, I'm not sure how good scaled up transformers can get, but
we're not done with trying to scale them up, and it's possible the next big inflection
point lies in better computers rather than better neural nets. I have a friend who runs
an ML compute startup, and he used to ask me "what would you do with 1000x more
...
> LLMs are not agentic enough to qualify as proto-AGI. Scaling them up won't lead
> to an AGI agent.

There's, just, a ton of people working on trying to turn LLMs into agents.
I was [working on it myself in 2023](https://auto-rt.github.io/)! My verdict so far
is that it's still a research prototype, but there's something there.
...
(Ugh you know the post is getting too long when you feel like it needs to quote itself.)

After seeing instruction finetuning work, I'm hesitant to say that agentic behavior cannot
emerge at scale with the right training dataset.
...
Humans grow up and learn from a huge host of stimuli and sensors. Models are doing
it differently. Large ML models do not have to learn the same ways humans do - but
is it possible to learn tasks that require real-world actions if you are not embodied
in the world where those actions will occur? The the

Or, using fewer fancy words: let's say you've never played tennis. If you read and
watch everything there is to know about tennis, then put your mind into the body
of Roger Federer, would you play tennis as well as Roger Federer?

This is the kind of thought experiment that is basically disprovable, making it a wonderful
topic if you want to say a lot of words about stuff. In robot learning, it's common to say
"no of course not". Conference of Robot Learning 2022 was held in Auckland, and while there
I had a chat with a professor who worked in ML, but didn't do any robotics. He was visiting
just because he worked nearby and thought it'd be interesting. We got on the topic
of embodiment, and he said "robotics researchers have a religion of embodiment", which I
quickly agreed was true.

This has always left me a bit of an odd duck in robotics, because I'm not a member of the
Church of Embodiment.
I'm not as far as the "embodiment is a waste of time" crowd. It's very obvious to me
that embodied data is special. It's inherently agentic, but also places a lot of focus
on physical reasoning. It's not obvious to me that embodied data is the only way to get
the capabilitie

It's very *not-obvious* to me that embodiment data is the only way for a model
is the only route
towards understanding how to take actions in the real world.
(But see [alternative views hera](https://keerthanapg.com/tech/embodiment-agi/).)

The analogy is that

AI safety people I know are often concerned about whether larger and larger transformers
will eventually attain the notion of understanding they are an agent in the world, that is
trained and deployed a certain way. They're concerned because it could promote deceptive
behavior, or hiding bad behavior at evalution time like the
