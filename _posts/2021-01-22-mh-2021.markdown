...
I'd argue that's bad because people don't want to make their teams much bigger.
...
Normally, teammate uses a "invite your friends" system, and through friend-of-friend-of-friends
...
moving from a heavily scripted Google Sheet
to a mini webapp with Discord integration. There were some rough edges, but it helped a lot.
The killer app was that when unlocked a puzzle, the bot would auto-create text and voice
...
then consider adding Nerts as a new source of "NO I NEEDED THAT".
...
you want, and if you're rate-limited, let the team know and we'll coordinate it then. This time we suggested
...
This never ended up being an issue for us - at the tiem of meta unlock, we usually still needed more
...
This was my favorite round of the Hunt. It was just so *weird*. The round technically has 100000
...
image was just too small and didn't have clear steganography inroads.)
...
even as the person proposing it later admitted they didn't think it would work.
...
parse to me, but then someone made this image for Paul Krugman:
...
Also psssst go watch the [TikTok](https://www.tiktok.com/@abhijitmudigonda/video/6918690407828311301?lang=en&is_from_webapp=v1&is_copy_url=1) we submitted, it's actually so good.
...
green circls / blue eyes from Sans, etc. All garbage! 
