...
MAYBE SEPARATE SECTION

Originally, this section was nine paragraphs where I explained various reasons
for why I was okay with the philosophical argument and didn't find it particularly
objectionable. Then I talked to several people who are very emphatic about AI
safety, and realized I was missing the point.

I think there's a massive meme about people who try to advocate for AI safety:
they either fully endorse the arguments of Nick Bostrom / Eliezer Yudkowsky /
Stuart Russell, or they reject them. I don't think that meme is true.

What I've seen is that everybody has their own complicated narrative
on why people should care about AI safety. Intelligence explosion and value
alignment are certainly common themes, ones that have been discussed to death,
but not everybody think it's particularly concerning to think about.
but not everyone agrees that's going to
...
For now, let's assume that by magic, there is a powerful enough machine
learning system. Then sure, the outcome described above is a possibility.
I think it's natural that simple objectives lead to complicated behavior - you
see this all the time in ML.

I also think it's natural that optimizing simple objectives doesn't guarantee
it does what you want. Again, this happens in ML all the time. The most
famous example is the tank classiifer. The army contracts a research lab
to classify images as tank or not-tank. They got good results, try it in a
real-world setting, and it does awful. Why?
All the positive examples were taken in daytime, and all the negative ones
were taken at night.
The model decided that all bright images have tanks and all dark images don't
have tanks. Garbage in, garbage out.

IMAGE HERE

I model machine learning algorithms as learning the easiest thing necessary to
get good performance, and nothing more. I've found this to be pretty useful,
especially for reinforcement learning. I've been burned by RL enough times that
I treat it as almost adversarial - if you're trying to design a reward function,
it is safer to assume your network will do its hardest to interpret it as
pathologically as possible.

Let me give an example: you're trying to use RL to teach a simulated robot arm
to pick up a block. So first, you give reward for making the block go up. To
avoid making the robot flail too much, you give a minor penalty for moving too
quickly.

The policy tries to learn how to get reward, but the motion to grab and lift
the block is too complex, and every time it tries to move, it gets penalized.
Eventually, the policy learns to avoid moving at all.

Okay, so now you add a shaped reward, where you get increasing reward as the
arm gets close to the block, then you give even more reward for lifting
the block into the air. You train the policy, and it learns to reach towards
the block, then stop. What happened now? This time, it first learned how to
reach towards a point. Once it got there, it did some exploration, and found that
whenever it touched the block, it got negative reward, because it never figured
out how to pick up the block. Over time, the policy learns "touching block bad"
and decides to stop touching the block.

Now, if you scale everything right, you can eventually use just self-play to
learn how to pick up a block, but defining a good reward for it is a lot harder
than it should be. There's a reason the block stacking papers from OpenAI use
a hardcoded policy or imitation learning - the top-down reward function approach
can be really finicky.

There are more examples of this too. The Eurisko bot, which was told to solve
a naval war game, and decided to field a massive army of tiny, immobile frigates,
along with one very fast submarine, instead of any sort of conventional army.
Then the tournament organizers added a "mobility" rule, and the Eurisko bot
decided *shooting its own ships* was the best way to meet the mobility
requirement. The ???? basketball team, which made the finals by playing legal-but-unconventional
basketball against teams that weren't prepared for it. (Both of these examples
are from a Malcolm Gladwell article that's a fun read.)

That was a bit of a tangent, but in short, I find it obvious that systems can
have outcomes that don't match their intention, because that happens already.
I find it obvious that machine learning systems in particular can do the same,
because that's happened already. So yes, if you want to posit a sufficiently
powerful AI, that's so capable it might as well be God, and then say that maybe
that God-AI may not do what we expect it to, then go right ahead - I won't get in
your way.

Aside: Miscellanerous Thoughts
------------------------------------------------------------------------------

When people talk about AI safety, it feels like they rally, really like to harp
on value alignment as if it's novel, exciting, and illuminating. Every time this
happens, I get bored, because I think it's none of those things, and people get too
swept-up in the narrative.

One common counterargument is the orthogonality thesis. People who believe it think
that intelligence and empathy do not go hand in hand, while people who reject it believe
intelligent beings are natural empathetic (and therefore superhuman AGI isn't a concern
because it'll care about us.) I've never liked reading debates on this, because
it often feels like people lean on too much philosophy, and use imprecise or incomplete
descriptions of imaginary objects to construct some logical argument that you're
supposed to find infallible.
And because the inputs are fuzzy, it quickly turns into unverifiable theorizing,
at which point I wonder why I should care.

Another common counterargument is that it would be easy to stop a superhuman AGI
system from doing anything we don't want it to do, simply by keeping it sandboxed from
the Internet. Again, I mostly consider these arguments a distraction, because the
AI-safety-apologist can simply imagine an AGI is powerful enough to make these
arguments invalid. "Something something intelligence-explosion-then-AGI-argues-you-should-let-it-out-of-the-box".
This doesn't feel like a productive line of argument, because again, it quickly
turns into arguments about assumptions about a thing that doesn't exist, which,
again, feels like it has enough margin for error that it doesn't feel useful.

However, I will grant this: that line of argument suggests there's some chance
that the safety problems are hard, and therefore people should be considering them
now. This applies to the earlier part too, and in fact is important enough that I
should emphasize it:

**Not everyone believes that the future will go the way the narrative says it will.
Rather, the arguments inspire thinking and introspection of a similar flavor, and at the
end some people become convinced.**

Or to be more precise,

**I would be profoundly disappointed if people were simply parroting the arguments
they heard. So far I haven't interacted with anyone who hasn't thought about it
themselves, but maybe I've gotten lucky with who I talk to.**


Thoughts on Timelines
------------------------------------------------------------------------------

I would like to preface this section by saying I find timelines distracting and
vaguely annoying to talk about.
...
they care! But not everybody's likes futurist speculation. Personally, it's
something I can only handle in moderation.

Everyone gets very excited about AGI, but it often feels like people are just
philosophizing about something in a way that can't be verified or refuted,
unless you're willing to wait 10 years. It's like people are reading tea leaves,
where every research paper is another leaf.

Overwhelmingly, every survey has shown that there is no consensus and massive
variance on when people expect tasks to be solved. This holds on randomly
selected NIPS attendees, on randomly selected AAAI attendees, on AAAI attendees
who are specifically going to workshops about AGI, and even on MIRI researchers.
At that point, why even bother asking? It feels like the value of information
is basically zero. If you can roll some dice, and get the same answer, you
haven't actually learned anything.

Generally, the actual number is much less important than the model that generates
the number, but even on this front I'm not sold there's a lot of useful
information. Predicting techonlogical progress is hard. I think everyone can
agree on that. Often, the models behind timelines are based on gut feelings about
the importance of software vs hardware, and whether areas of research will
continue to be fruitful or not. This also has lots of randomness, so even if you
focus on the model behind the timeline, the end result is still very subjective.

This is a long way of saying that discussion of these topics isn't well suited
to mediums that don't support a back-and-forth conversation.

Whenever I talk to EAs, I see a frustrating tendency for people to condense
an entire ML researcher into just their timeline and opinion about AGI. Stuart
Russell is "that guy who agrees with us"; Andrew Ng is "that guy who made
that Mars analogy."
